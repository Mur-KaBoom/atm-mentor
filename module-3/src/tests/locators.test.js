...
describe('Module 3: WebdriverIO Introduction', () => {
  ...
  it('Test-2 edit doctor', async () => {
    ...
    await $("//input[contains (@name, 'Education')]").setValue('MD, DM, FACC, FICC,КПИ, УИПА, ХПИ')
    ...
    await expect($('div.basic-detail.info-field-container div.education')).toHaveText('MD, DM, FACC, FICC,КПИ, УИПА, ХПИ')
  });
  ...
  it('Test-4 edit appointment', async () => {
    ...
    await expect($("//div[contains(@aria-label, 'Laura Begin From Wednesday, August 5, 2020 at 1:00:00 PM')]//div[contains(@class, 'e-location')]")).toHaveText('London')
  });

  it('Test-5 delete doctor', async () => {
    await browser.url('/showcase/angular/appointmentplanner/#/dashboard')
    await $("div[routerLink='/doctors']").click()

    const expectedDoctors = ['Dr. Nembo Lukeni', 'Dr. Mollie Cobb', 'Dr. Yara Barros', 'Dr. Paul Walker', 'Dr. Amelia Edwards', 'Dr. Alexa Richardson', 'Dr. Nout Golstein']
    const actualDoctors = []
    const navDoctorLinks = await $$("//div[contains(@class, 'specialist-display')]//div[contains(@id, 'Specialist')]//div[contains(@class, 'name')]")
